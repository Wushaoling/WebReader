...
namespace WebReader.Service
{
    public class FileService
    {
        ...
        //获得所有的文件
        public List<Models.MyFile> GetFiles(String author)
        {
            String sql = null;
            if (null == author)
            ...
            List<Models.MyFile> list = ToList(dt, dt.Rows.Count);
            dbService.Close();
            return list;
        }

        //根据上传时间排序
        public List<Models.MyFile> GetTimeOrder(String author)
        {
            String sql = null;
            if (null == author)
            ...
            DBService dbService = new DBService();
            DataTable dt = dbService.Query(sql);
            List<Models.MyFile> list = ToList(dt, 10);
            dbService.Close();
            return list;
        }

        //根据阅读量排序
        public List<Models.MyFile> GetDownloadOrder(String author)
        {
            String sql = null;
            if (null == author)
        ...
        //搜索文件
        public List<Models.MyFile> Search(String keyword, String author)
        {
            String s = null;
            if (null == author)
                s = " ispublic = true and ";
        ...
        //将DataTable转为List<>
        public List<Models.MyFile> ToList(DataTable dt, int len)
        {
            int count = 1;
            List<Models.MyFile> list = new List<Models.MyFile>();
            foreach (DataRow dr in dt.Rows)
            {
                if (count++ > len)
                    break;
                Models.MyFile f = new Models.MyFile();
                f.setID((int)dr["ID"]);
